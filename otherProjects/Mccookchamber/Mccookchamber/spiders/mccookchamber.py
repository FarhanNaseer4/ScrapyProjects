...
class MccookchamberSpider(scrapy.Spider):
    name = 'mccookchamber'
    start_urls = ['https://members.mccookchamber.org/members/FindStartsWith?term=%23%21']

    custom_settings = {
        'FEED_URI': 'mccookchamber.csv',
        'FEED_FORMAT': 'csv',
    ...
    def detail_page(self, response):
        try:
            ...
            item['Business_Site'] = response.css('li.gz-details-website a::attr(href)').get('').strip()
            item['Social_Media'] = ', '.join(
                data.css('::attr(href)').get('') for data in response.css('li.gz-details-social a'))
            item['Source_URL'] = 'https://members.mccookchamber.org/members/FindStartsWith?term=%23%21'
            ...
            item['Lead_Source'] = 'mccookchamber'
