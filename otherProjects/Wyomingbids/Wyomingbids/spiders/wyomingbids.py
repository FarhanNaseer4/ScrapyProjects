...
class WyomingbidsSpider(scrapy.Spider):
    name = 'wyomingbids'
    start_urls = ['https://www.wyomingbids.us/wyoming-contractors/category.htm']

    base_url = 'https://www.wyomingbids.us{}'
    custom_settings = {
        'FEED_URI': 'wyomingbids.csv',
        'FEED_FORMAT': 'csv',
    ...
    def listing_page(self, response):
        for data in response.css('div.innertube div.list-one'):
            ...
            if detail_url:
                ...
                item['Source_URL'] = 'https://www.wyomingbids.us/wyoming-contractors/category.htm'
                item['Lead_Source'] = 'wyomingbids'
                item['Occupation'] = 'Contractor'
                item['Record_Type'] = 'Person'
                item['Meta_Description'] = "Contractors in Wyoming"
    ...
    def detail_page(self, response):
        ...
        fullname = self.get_name_parts(response.xpath('//dt[contains(text(),"Contact")]/following-sibling::dd/text()').get('').strip())
