...
class KearneycocSpider(scrapy.Spider):
    name = 'kearneycoc'
    start_urls = ['https://chambermaster.kearneycoc.org/list']

    custom_settings = {
        'FEED_URI': 'kearneycoc.csv',
        'FEED_FORMAT': 'csv',
    ...
    def detail_page(self, response):
        try:
            ...
            item['Social_Media'] = ', '.join(
                data.css('::attr(href)').get('') for data in response.css('div#mn-memberinfo-block-socialnetworks a'))
            item['Source_URL'] = 'https://chambermaster.kearneycoc.org/list'
            occupy = response.css('ul.mn-member-cats li::text').get('').strip()
            if occupy:
